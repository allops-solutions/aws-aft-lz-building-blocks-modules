...
resource "aws_s3_bucket" "pipeline" {
  ...
  bucket = "${var.solution_name}-cb-${data.aws_caller_identity.current.account_id}-${data.aws_region.current.name}"
  tags   = var.tags
}
...
resource "aws_s3_bucket" "tf_backend" {
  ...
  #checkov:skip=CKV_AWS_144: This bucket does not need cross-region replication enabled
  #checkov:skip=CKV_AWS_18: This bucket does not need access logging enabled
  bucket = "aws-identity-mgmt-tf-${data.aws_caller_identity.current.account_id}-${data.aws_region.current.name}"
  tags   = var.tags
}
