...
resource "aws_cloudwatch_event_rule" "pipeline" {
  ...
  description    = "Capture Control Tower events to start the identity and access management pipeline"
  event_bus_name = aws_cloudwatch_event_bus.pipeline[0].name
...
resource "aws_cloudwatch_event_rule" "ct_events" {
  ...
  description = "Capture Control Tower events to send to the identity and access management pipeline event bus"
  event_pattern = jsonencode(
    {
